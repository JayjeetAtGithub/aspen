...
auto build_large_compressed_kv_graph(string fname, size_t n_parts=1) {
...
auto initialize_kv_graph(string fname="", bool mmap=false, bool compressed=false, size_t n_parts=1) {
  if (fname == "") {
    cout << "Unimplemented!" << endl;
    exit(0);
  }
  // if (compressed && n_parts > 1) {
  //   cout << "Building large compressed graph" << endl;
  //   return build_large_compressed_kv_graph(fname, n_parts);
  // }
  size_t n;
  uintK* keys; strV* values;
  if (!compressed) {
    cout << "Reading Unweighted Graph" << endl;
    std::tie(n, keys, values) = read_unweighted_kv_graph(fname.c_str(), mmap);
    cout << "Read Unweighted Graph" << endl;
  }
  // } else {
  //   cout << "Reading Compressed Graph" << endl;
  //   std::tie(n, keys, values) = read_compressed_kv_graph(fname.c_str(), mmap);
  //   cout << "Read Compressed Graph" << endl;
  // }
