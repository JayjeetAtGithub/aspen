...
void parallel_updates(commandLine& P) {
  ...
  //     auto updates = pbbs::sequence<pair_kv_vertex>(updates_to_run);

  //     double a = 0.5;
  //     double b = 0.1;
  //     double c = 0.1;
  //     size_t nn = 1 << (pbbs::log2_up(n) - 1);
  //     auto rmat = rMat<strV>(nn, r.ith_rand(0), a, b, c);
  ...
  //       //cout << "Inserting" << endl;
  //       timer st; st.start();
  //       VG.insert_kv_edges_batch(update_sizes[us], updates.begin(), false, true, nn, false);
  ...
  //       // cout << "Deleting" << endl;
  //       timer st; st.start();
  //       VG.delete_kv_edges_batch(update_sizes[us], updates.begin(), false, true, nn, false);
  //       double batch_time = st.stop();
